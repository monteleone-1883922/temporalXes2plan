"""Tests for encoding.xor_guard_duplicator.XorGuardDuplicator."""
import pytest
...
class TestSingleClauseGuard:
    ...
    def test_no_duplication_with_single_clause(self, xor_parse_result_single_clause):
        ...
        XorGuardDuplicator(xor_parse_result_single_clause).apply(registry)
...
class TestMultiClauseGuard:
    ...
    def test_two_or_clauses_produce_two_variants(self, xor_parse_result_multi_clause):
        ...
        XorGuardDuplicator(xor_parse_result_multi_clause).apply(registry)
...
class TestNoGuards:
    ...
    def test_cascade_level2_no_conditions_added(self, xor_parse_result_no_guards):
        registry = _make_registry(xor_parse_result_no_guards)
        original_left = registry.get("execute_left_branch")[0]
        original_preconds = list(original_left.preconditions)

        XorGuardDuplicator(xor_parse_result_no_guards).apply(registry)

        variant = registry.get("execute_left_branch")[0]
        assert variant.preconditions == original_preconds

    def test_cascade_level2_no_duplication(self, xor_parse_result_no_guards):
        registry = _make_registry(xor_parse_result_no_guards)
        XorGuardDuplicator(xor_parse_result_no_guards).apply(registry)
...
class TestCartesianProduct:
    ...
    def test_existing_two_variants_times_two_clauses(self, xor_parse_result_multi_clause):
        ...
        # Manually pre-expand left_branch to 2 variants (simulates a prior phase)
