...
// ─────────────────────────────────────────────────────────────────────────────
// Artificial XOR split panel
// ─────────────────────────────────────────────────────────────────────────────

function showArtificialXorPanel(nodeId) {
    if (!currentData || !currentData.artificial_xor_splits) return;
    const art = currentData.artificial_xor_splits[nodeId];
    if (!art) return;

    const icon = document.getElementById("panel-header-icon");
    icon.className = "panel-header-icon artificial";
    icon.innerHTML = '<i class="bi bi-cpu"></i>';

    document.getElementById("panel-title").textContent = `Variants: ${art.source_activity}`;
    const content = document.getElementById("panel-content");
    content.innerHTML = "";

    const intro = document.createElement("p");
    intro.className = "empty-note";
    intro.style.cssText = "margin-bottom:12px;font-style:normal;color:#6b21a8;";
    intro.innerHTML = '<i class="bi bi-info-circle" style="margin-right:4px;"></i>Artificial split — created by effect duplication during encoding.';
    content.appendChild(intro);

    for (const branch of art.branches) {
        const card = document.createElement("div");
        card.className = "branch-card artificial";
        ...
        const name = document.createElement("span");
        name.className = "branch-name";
        name.textContent = branch.variant_name;
        const badge = document.createElement("span");
        badge.className = "prob-badge";
        ...
        // Distinguishing preconditions
        if (branch.conditions && branch.conditions.length > 0) {
            const label = document.createElement("div");
            label.className = "cond-label";
            label.textContent = "Variant Conditions";
            card.appendChild(label);
            for (let i = 0; i < branch.conditions.length; i++) {
                if (i > 0) card.appendChild(orSeparator());
                const group = document.createElement("div");
        ...
        if (branch.variant_effects && branch.variant_effects.length > 0) {
            const label = document.createElement("div");
            label.className = "cond-label";
            label.style.marginTop = "8px";
            label.textContent = "Variant Effects";
            card.appendChild(label);
            const row = document.createElement("div");
            row.style.cssText = "display:flex;flex-wrap:wrap;gap:4px;margin-top:4px;";
            for (const eff of branch.variant_effects) {
                const pill = document.createElement("span");
                pill.className = "variant-effect-pill";
                pill.textContent = `${eff.attribute} ${eff.predicate} ${eff.value}`;
        ...
        if ((!branch.conditions || branch.conditions.length === 0) &&
            (!branch.variant_effects || branch.variant_effects.length === 0)) {
            card.appendChild(emptyNote("No distinguishing conditions or effects"));
        }
